...
# for obsrevation
def time_to_deg(h: float = 0, m: float = 0, s: float = 0) -> float:
    """
        convert time to deg
    """
    h += m/60 + s/360
...
# for obsrevation
def dec_to_deg(d: float = 0, m: float = 0, s: float = 0) -> float:
    """
...
if __name__ == '__main__':
    ...
    print(
        f"B[phy]: {b_phy:.2e} [sqrt(Msun)*km/(s*pc^1.5)] = {b_G:.4e} [G,sqrt(g/cm)/s]")
