...
def calc_connected_structure_3d(data, start_i, start_j, start_k, threshold, indices, min_threshold):
    """
...
def get_boundary_index_3d(data: any, threshold: float) -> tuple(any, float):
    """
...
def binary_search_bondory_3d(data:any, min_threshold, max_threshold) -> any:
    """
    ...
    i_bdry, j_bdry, k_bdry, indices = calc_connected_structure_3d(data, i_bfs, j_bfs, k_bfs, threshold, indices, min_threshold)
    ...
    while True:    
        ...
        i_bdry_next, j_bdry_next, k_bdry_next, indices_next = calc_connected_structure_3d(data, i_bdry, j_bdry, k_bdry, threshold, indices, min_threshold)
...
def binary_search_bondory_by_mass_3d(data: any, min_threshold: float, max_threshold: float, target_mass: float) -> any:
    """
    ...
    if curr_mass  > target_mass:
        print(f"maximum threshold gives mass = {curr_mass:2f}, which is too low")
        return
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        with h5py.File(file, 'r') as data:
            ...
            # print(data['xc1'][:][1] - data['xc1'][:][0]) 
            # sys.exit("end")

            # density 
            density = data['den'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)]
            # momentum
            momx = data['momx'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)]
            momy = data['momy'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)]
            momz = data['momz'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)]
            # magnetic field
            bx = 0.5*(data['bxr'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)] + data['bxl'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)])
            by = 0.5*(data['byr'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)] + data['byl'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)])
            bz = 0.5*(data['bzr'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)] + data['bzl'][nbuf:(nz + nbuf), nbuf:(ny + nbuf), nbuf:(nx + nbuf)])
            # cooridinates
        ...
        # use this line if needed to find the boundary.
        index, threshold = binary_search_bondory_by_mass_3d(density, thresholds[1], 36, target_mass=target_mass[0])
        ###################################################################
        ...
        # write output file.
        newFileNAME = file.replace(".h5", "_data_in_contour_new_contour_by_mass.h5")
